        ...
        try {
            ...
            foreach ($attendeesData as $key => $value) {
                if ($request->hasFile("attendees.$key")) {
                   ...
                   if ($key === 'Attendees') {
                    $attendee->$key = json_encode($value);
                } else {
                    $attendee->$key = $value;
                }
                }
    ...
    public function attendyList(Request $request, $userId, $event_id)
    {
        try {
            ...
            // $finalList = $corporates->concat($attendees)->values();
                      ...
                      // 🔹 Ticket-wise attendee counts (Booking + Agent, based on attendee_id)
            $ticketsData = Ticket::where('event_id', $event_id)
                ->get()
                ->map(function ($ticket) {
                    // Count unique attendees from Booking table
                    $bookingCount = Booking::where('ticket_id', $ticket->id)
                        ->distinct('attendee_id')
                        ->count('attendee_id');

                    // Count unique attendees from Agent table
                    $agentCount = Agent::where('ticket_id', $ticket->id)
                        ->distinct('attendee_id')
                        ->count('attendee_id');
